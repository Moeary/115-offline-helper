...
> 后台监控：推送成功后，任务会携带明确的 `processorProfile`。`generic` 仅做安全清理，`jav` 可继续执行番号整理，`anime` 默认保留 torrent 原名；同一个确认窗口批量提交的 anime 任务会在下载完成后把视频/字幕移到所选保存目录，并仅在确认为空时删除任务文件夹，不创建或强制改名。网站默认值可在确认窗中覆盖；后台不会根据 hostname 猜测规则。
